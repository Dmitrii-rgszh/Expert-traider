...
- Верстка должна быть адаптивной, ориентированной на Telegram Mini App: всегда проверяйте отображение в webview, все элементы должны помещаться в экран без горизонтальной прокрутки.
- При добавлении новых блоков сначала проектируйте мобильный сценарий (ширина 320–390 px), затем масштабируйте до десктопа.
- Используйте чистую, «вкусную» визуальную стилистику: мягкие тени, читаемые шрифты, единый набор цветов и отступов.
- Любые новые компоненты должны следовать выбранной дизайн-системе (цвета, шрифты, размеры кнопок, состояния наведения/нажатия).
- Перед публикацией проверяйте макет в настоящем Telegram Mini App (через туннель) и обновляйте гайды, если появляются новые паттерны.
